...
export function addScaled(point, direction, scalar) {
    return {
...
export function rotatePoint(point, pivot, angle) {
    ...
    const rotated = rotateVector(dx, dy, angle);
...
export function angleLerp(from, to, t) {
    return normalizeAngle(
        from + shortestAngleDelta(from, to) * clamp01(t)
    );
}
...
export function nearlyEqual(a, b, epsilon = 0.0001) {
    return Math.abs(a - b) <= epsilon;
}

export function nearlyEqualPoints(a, b, epsilon = 0.0001) {
    return (
...
export function finitePoint(point, fallbackX = 0, fallbackY = 0) {
    return {
        x: finite(point?.x, fallbackX),
        y: finite(point?.y, fallbackY)
    };
}

export function safeHypot(x, y) {
    const result = Math.hypot(x, y);

    return Number.isFinite(result) ? result : 0;
}

export function signNonZero(value, fallback = 1) {
    if (value > EPSILON) {
        return 1;
    }
